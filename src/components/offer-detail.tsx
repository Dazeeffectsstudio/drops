...
export function OfferDetail({ offer }: { offer: Offer }) {
  ...
  useEffect(() => { const current = Date.now(); setNow(current); setDemoStart(Number(sessionStorage.getItem("drops-demo-start")) || current); const timer = window.setInterval(() => setNow(Date.now()), 30_000); return () => window.clearInterval(timer); }, []);
  const expiration = offerExpiresAt(demoStart, offer);
